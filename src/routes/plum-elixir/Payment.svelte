<script lang="ts">
    ...
    import { readHighestBid, bid } from '$lib/asyncActions';

    const highestBidQuery = createQuery({
      queryKey: ['current_bid'],
      queryFn: readHighestBid,
      refetchInterval: 4000,
    });

    let highestBid = '-';
    highestBidQuery.subscribe((value) => {
      if (value.isError || value.isLoading) return '-';
      highestBid = value.data!.toString();
    });

    const queryClient = useQueryClient();

    const bidMutation = createMutation({
      mutationFn: bid,
      onSuccess: () => {
	...
	async function makeOffer() {
            toggleModal();
            $bidMutation.mutate(amountInXRP);
          };
